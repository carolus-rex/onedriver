...
class DirCommand(BaseCommand):
    ...
    def __init__(self, drive):
        ...
        self.parser.add_argument('/C', '/c',
                                 help="Shows the thousand separator in file size. "
                                      "This is the default option. Use /-C to disable.",
                                 action="store_true",
    ...
    def get_max_item_name_length(self, items):
        sort_key = lambda item: len(item.name) if item.folder is None else len(item.name) + 2
